...
if [ -z $1 ]; then
 ...
 echo "-b ........ would start sbovl-licsbas (use dev version here..)"
 echo "-q ........ if sbovl-licsbas, will rerun again to check high residual in first inversion (LiCSBAS13)" it is really usefull especially mid-coherent region like Turkiye frames.
...
while getopts ":M:h:HucTsdbSlWgmaNAiIeFfOBPpRrLwkXxC:G:E:t:n:QZq" option; do
 case "${option}" in
  ...
  M) multi=${OPTARG};
     ...
     ;;
  b) sbovl=1;
     echo "setting to process sbovl (Subswath and Burst Overlap Interferometry) data, default mode is referenced along-track displacement."
     ;;
  x) sbovl_model=1;
...
if [ $sbovl -gt 0 ]; then
 echo "using sbovl - disabling -u -g etc (if set)"
 reunw=0
 dogacos=0
