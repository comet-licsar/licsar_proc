...
if [ $do_ifg -eq 1 ]; then
 ...
 #to flatten it:
 convert $ifg_bmp PNG8:temptemp.png
...
if [ $do_ifgu -eq 1 ]; then
 #perhaps we are to add unfiltered one:
  if [ -f $pair.geo.diff_unfiltered_pha.tif ]; then
    ...
    convert $ifg_unfiltered_bmp PNG8:temptemp.png
...
if [ $do_unw -eq 1 ]; then
  ...
  if [ -f $hgtfile ]; then
    ...
    gmt grdgradient -A$deg -Nt1 -Rtemp.nc -Ghillsh.tif=gd:GTiff $hgtfile
    hillshadecmd="-Ihillsh.tif"
  fi
  #generate figure, incl. hillshade
  gmt grdimage temp.nc -Cunw.cpt -JM1 -Q -nn+t0.1 $hillshadecmd -Atempunwbmp.png
  if [ ! -f tempunwbmp.png ]; then
