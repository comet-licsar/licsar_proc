...
heredir=`pwd`
ifgdir=$heredir/$frame/GEOC/$ifgid
...
#lets not use landmask for overlaps
#maskfile=$heredir/$frame/GEOC/$frame.geo.landmask.ovlp.tif
ifg=$ifgdir/$ifgid.geo.diff_pha.ovlp.tif
coh=$ifgdir/$ifgid.geo.cc.ovlp.tif
outunw=$ifgdir/$ifgid.geo.unw.ovlp.tif
...
gmt grdconvert temp/mask.fullin.georeg.nc temp/mask.fullin.georeg.tif=gd:GTiff
gmt grdconvert temp/mask_snaphu.nc temp/mask.snaphu.tif=gd:GTiff
gmt grdconvert temp/mask.nc temp/maskt.tif=gd:GTiff
gmt grdconvert temp/ifg.nc temp/ifg.tif=gd:GTiff
gdalwarp2match.py temp/mask.fullin.georeg.tif temp/ifg.tif temp/mask.fullin.georeg2.tif
gdalwarp2match.py temp/mask.snaphu.tif temp/ifg.tif temp/mask_snaphu.tif
gdalwarp2match.py temp/maskt.tif temp/ifg.tif temp/mask.tif
gdalwarp2match.py $coh temp/ifg.tif temp/coh.tif
gmt grdconvert temp/mask.fullin.georeg2.tif temp/mask.fullin.georeg.nc
gmt grdconvert temp/mask_snaphu.tif temp/mask_snaphu.nc
gmt grdconvert temp/mask.tif temp/mask.nc
coh=temp/coh.tif

if [ `gmt grdinfo temp/mask.fullin.georeg.nc | grep -c Pixel` -lt 1 ]; then
 gmt grdedit temp/mask.fullin.georeg.nc -T -R$ifg   #to pixel reg
fi
if [ `gmt grdinfo temp/ifg.nc | grep -c Pixel` -lt 1 ]; then
 gmt grdedit temp/ifg.nc -T -R$ifg   #to pixel reg
fi
...
###gmt grdmath temp/ifg.nc PI ADD = temp/pha1.02pi.nc    # without landmasking
