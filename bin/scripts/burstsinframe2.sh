...
#
#  Dependencies:
#   extract_bursts_loc_v3.0.py, listlonlat2mgrs.py
#
...
usage() { 
  ...
  echo "   e.g. burstsinframe.sh A14frame3.xy A14frame3_burst_ids.txt /nfs/a1/raw/sentinel/T014A/20151130 151130 " 1>&2; 
  echo " " 1>&2; 
  exit 1; 
}
...
ls -d $SLCdir/S1*$date*.zip > tmp_ziplist2.txt
for file in `more tmp_ziplist2.txt`; do
    ...
    cat allcenterIW1.list coord1names.list > tmp1.txt; mv tmp1.txt allcenterIW1.list
    awk '{print $1, $2, $3, 2, "'$file'"}' coordcenterIW2.list > coord2names.list
    cat allcenterIW2.list coord2names.list > tmp2.txt; mv tmp2.txt allcenterIW2.list
    awk '{print $1, $2, $3, 3, "'$file'"}' coordcenterIW3.list > coord3names.list
    cat allcenterIW3.list coord3names.list > tmp3.txt; mv tmp3.txt allcenterIW3.list
...
for i in `seq 1 $nbursts`; do
  ...
  #this line to debug
  #awk '{print '$i',$2,'$lon',$3,'$lat', $2-"'"${lon}"'",111*sqrt((($2-"'"$lon"'")*cos("'"$lat"'"*(3.14159/180)))^2 + ($3-"'"$lat"'")^2)}' allcenter.list
...
# Check if the centers are more than 5 km apart, then we consider the bursts are the same burst.
