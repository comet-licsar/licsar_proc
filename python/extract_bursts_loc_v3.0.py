#!/usr/bin/env python
# -*- coding: iso-8859-1 -*-

import sys, zipfile, os, re
import xml.etree.ElementTree as et

def hhmmss2secofday(HH,MM,SS):
  secofday = HH*3600 + MM*60 + SS
  return secofday

def usage():
  print """
  ### Read a zip file or .SAFE dir and extract info for existing bursts v3.0 3-Dec-2105 RJW
  ###                                                      v2.0 30-Nov-2015 PJG
  ###                                                      v1.0 13-Oct-2015 PJG
  ###                                                      v0.1 12-Oct-2015 PJG
  
  extract_bursts_loc.py
    Program reads a compressed or noncompressed SAFE/ file to estimate central coordinates of each burst.
  
  Usage: extract_bursts_loc.py [file.zip/file.SAFE]
    zipfile   (input) name of the compressed Sentinel-1 SAFE/ file with .zip extension or a .SAFE dir
        
"""

if len(sys.argv) < 2:
  usage()
  sys.exit(-1)

infile = sys.argv[1]

if infile[-4:] == "SAFE":
  #print "Reading SAFE file"
  namelist = []
  for path, subdirs, files in os.walk(infile):
    for name in files:
        namelist = namelist + [os.path.join(path, name)]

if infile[-4:] == ".zip":
  #print "Reading zip file"  
  zipfilepath=sys.argv[1]
  zfile=zipfile.ZipFile(zipfilepath, 'r') # Open the zip file
  namelist = zfile.namelist()

# Identify the annotation files containing the coordinates of the bursts
list_annfile=['annotation/s1.-iw1-slc-vv', 'annotation/s1.-iw2-slc-vv', 'annotation/s1.-iw3-slc-vv']
for fnames in namelist:
  for annfile in list_annfile:
    if re.search(annfile,fnames):
      # Open the file and Read the xml structure
      try:
        if infile[-4:] == "SAFE":  
          doc = et.parse(fnames)
        if infile[-4:] == ".zip": 
          doc = et.parse(zfile.open(fnames))
        root = doc.getroot()
        # Extract the subswath info
        for headerinfo in root.findall('adsHeader'):
          swath=headerinfo.find('swath').text
        # Open files
        file0 = open("coordcenter"+swath+".list", "w")  # File central coordinates of bursts
        fileedge = open("coordedge"+swath+".list", "w") # File coordinates of bursts edges

        # Work out the central coordinates of each burst
        for geoGridinfo in root.findall('geolocationGrid'):
          for coordlist in geoGridinfo:
            npts = coordlist.get('count')
            #print "Number of Geotagged points: " + npts
            pts = 0; lo = [ ];  la = [ ]; t = [ ]
            for pt in coordlist:
              pts = pts+1
              lon = pt.find('longitude').text
              lat = pt.find('latitude').text
              line = pt.find('line').text
              pixel = pt.find('pixel').text
              azT = pt.find('azimuthTime').text # 2014-11-16T16:15:40.304137
              azHH=float(azT[11:12+1]); 
              azMM=float(azT[14:15+1]);
              azSS=float(azT[17:21+1]);
              #print azT, azHH, azMM, azSS
              azT = hhmmss2secofday(azHH,azMM,azSS);
              # The following assumes there are 21 points per line of geotagged points in xml files
              if pts % 21 == 0 or pixel == "0":
                lo = lo + [float(lon)] # Add the new line lon value to the lo list
                la = la + [float(lat)] # Add the new line lat value to the la list
                t = t + [float(azT)]  # Add the new line lat value to the la list
                # print lo,la,t         

        burstID = 0
        for i in range(3,len(lo)+1,2):
          burstID = burstID+1
          fileedge.write("> %s \n" % (burstID) )
          for j in [0, 1, 3, 2]:
            fileedge.write("%s %s \n" % (lo[i-j], la[i-j]) )
          # Calculate central coordinates of burst
          lon0 = (lo[i-3]+lo[i-2]+lo[i-1]+lo[i])/4
          lat0 = (la[i-3]+la[i-2]+la[i-1]+la[i])/4
          t0 = (t[i-3]+t[i-2]+t[i-1]+t[i])/4
          # Write central coordinates
          file0.write("%s %s %s %s\n" % (lon0, lat0, burstID, t0) )
        
        # Catch exception if root not available
        if root.tag == "root":
          print "I'm the root"

        # Close files
        file0.close()  
        fileedge.close()
      
      except et.ParseError:
        print(fnames)
        sys.exit(2)
#zfile.close() #Close zipfile
  
      
