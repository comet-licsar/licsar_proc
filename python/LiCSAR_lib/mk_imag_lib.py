...
#Read files function
################################################################################
def read_files( filelist, slcdir, imdate, procdir, licsQuery, job_id ):
    """
...
def make_frame_image( date, framename, burstlist, procdir, licsQuery, 
    ...
    This functions works by building up each swathe from multiple
...
#                        slctab = os.path.join(tabdir,'{0}_tab'.format(fileset[0].split('T')[0]))
