...
a[colname]=a[colname]-a[colname][i]

# now get the iono gradients
valsI = gradts(a,colname, resolm)
#valsT = gradts(a,'tide')
...
monthly_stats = df.groupby(df["date"].dt.to_period("M"))[colname+"grad"].agg(["mean", "std"])
