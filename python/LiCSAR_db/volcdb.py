...
try:
    ...
    from dbfunctions import Conn_db #sqlalchemy
...
import warnings
warnings.filterwarnings("ignore", message=".*DBAPI2 objects are not tested.*")
...
def create_volclip_for_volcano(volcid, cliparea_geo = None, allow_multiples = False):
    ...
    #
    # adding to volclips
    sql_q = "INSERT INTO volclips (vid, geometry) VALUES ({0}, ST_GeomFromText('{1}'));".format(str(vid), wkt)
    res = do_query(sql_q, True)
    #
