...
def usage():
  print """
### Read two burst coordinate lists and extract common bursts filtered by coordinates
    reporting first and last common bursts  v0.1 19-Oct-2015 PJG'  
  
BurstList2CommonGeoBursts.py
  ...
  filtered by the ones falling within a given coordinate range
    
  Usage: BurstList2CommonGeoBursts.py master_burstlist slave_burstlist
    master_burstlist  (input) filename master burst list with format [long, lat, burstID]
     slave_burstlist  (input) filename slave burst list with format [long, lat, burstID]
                west  (input) minimum longitude coordinates 
                east  (input) maximum longitude coordinates
               north  (input) minimum latitude coordinates 
               south  (input) maximum latitude coordinates                 
...
south=float(sys.argv[6])
polygon = points2polygon(west,east,north,south)
