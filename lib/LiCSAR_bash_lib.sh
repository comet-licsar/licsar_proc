...
function create_LOS_tide_frame_allepochs() {
  ...
  local track=$(echo "$frame" | cut -c -3 | sed 's/^0//')
  local epochspath="$LiCSAR_public/$track/$frame/epochs"

  if [ ! -d "$epochspath" ]; then
    echo "Error: Epochs directory does not exist: $epochspath"
    return 1
  fi
...
function create_LOS_tide() {
  ...
  local datem1=$(date -d "$date1" +%Y-%m-%d)
  local track=$(echo "$frame" | cut -c -3 | sed 's/^0//')

  # Get center time
  local epochpath="$LiCSAR_public/$track/$frame/epochs/$date1"

  # Determine the correct tide file to check
  local tide_file="$epochpath/$date1.tide.geo.tif"
  ...
  local U="$LiCSAR_public/$track/$frame/metadata/$frame.geo.U.tif"
  local E="$LiCSAR_public/$track/$frame/metadata/$frame.geo.E.tif"
  local N="$LiCSAR_public/$track/$frame/metadata/$frame.geo.N.tif"
...
function create_notide_noiono_012A() {
